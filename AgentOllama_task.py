...
def agentollama_task(prompt_value):
    ...
    def grade_documents(state):
        ...
        for d in documents:
            ...
            grade = score["score"]
            if grade == "yes":
                print("---ОЦЕНКА: ДОКУМЕНТ РЕЛЕВАНТНЫЙ---")
                filtered_docs.append(d)
            else:
                print("---ОЦЕНКА: ДОКУМЕНТ НЕРЕЛЕВАНТНЫЙ---")
                continue
    ...
    dataset = chain.invoke({"input": output})
...
if __name__ == '__main__':
    agentollama_task("Собрать датасет с значениями MolMR, Mollol, HEHEHEHEHE для молекул с высокой молекулярной рефракцией.")